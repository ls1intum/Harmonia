...
| ----------------------------- | ----------------------------------- | ----------- |
| [**streamData**](#streamdata) | **GET** /api/requestResource/stream |             |
...
```typescript
import { StreamResourceApi, Configuration } from './api';

const configuration = new Configuration();
const apiInstance = new StreamResourceApi(configuration);
...
const { status, data } = await apiInstance.streamData(exerciseId, jwt, artemisServerUrl, artemisUsername, artemisPassword);
```

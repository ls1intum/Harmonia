...
```typescript
import { CqiWeightResourceApi, Configuration } from './api';
...
const { status, data } = await apiInstance.getWeights(exerciseId);
```
...
const { status, data } = await apiInstance.resetWeights(exerciseId);
```
...
```typescript
import { CqiWeightResourceApi, Configuration, CqiWeightsDTO } from './api';
...
const { status, data } = await apiInstance.saveWeights(exerciseId, cqiWeightsDTO);
```
